...
public class Negativity {
	...
	/**
	 * Try to alert moderator. It will not check everything, but there is some
	 * verification which are made such as: - if the cheat is active - the
	 * reliability amount - Detection disabled on verification - Bypass with
	 * permissions - ...
	 * 
	 ...
	 * @return true if the player have to be set back
	 */
	public static boolean alertMod(ReportType type, Player p, Cheat c, int reliability, String checkName, String proof, CheatHover hover) {
	 ...
	 * @return true if the player have to be set back
	 */
	public static boolean alertMod(ReportType type, Player p, Cheat c, int reliability, String checkName, String proof, CheatHover hover, long amount) {
		...
		if (hasBypass && (Perm.hasPerm(NegativityPlayer.getNegativityPlayer(p), "bypass." + c.getKey().getLowerKey()) || Perm.hasPerm(NegativityPlayer.getNegativityPlayer(p), Perm.BYPASS_ALL))) {
	...
	private static void manageAlertCommand(NegativityPlayer np, ReportType type, Player p, Cheat c, int reliability) {
		...
		for (String s : conf.getStringList("run")) {
			Adapter.getAdapter().runConsoleCommand(UniversalUtils.replacePlaceholders(s, "%name%", p.getName(), "%uuid%", p.getUniqueId().toString(), "%cheat_key%", c.getKey().getLowerKey(),
					"%cheat_name%", c.getName(), "%reliability%", reliability, "%report_type%", type.name(), "%warn%", np.getWarn(c)));
		}
	}

	/**
	 * Send alert message to all player which can receive them If any, the message
	 * will keep in cache and showed when someone can see them If a lot of one, all
	 * message are compiled into one (and amount or are additionned)
	 * 
	 * @param np    the negativity player which create alert
	...
	public static void sendAlertMessage(NegativityPlayer np, PlayerCheatAlertEvent alert) {
		...
		if (alert.getNbAlertConsole() > 0 && log_console) {
			...
			ada.getLogger().info("New " + alert.getReportType().getName() + " for " + p.getName() + " (" + ping + " ms, UUID: " + p.getUniqueId() + ") seem to use " + c.getName() + " "
		...
		if (!alert.getReportType().equals(ReportType.INFO))
			WebhookManager.addToQueue(new AlertWebhookMessage(WebhookMessageType.ALERT, p, "Negativity", System.currentTimeMillis(), alert.getNbAlert() == 0 ? 1 : alert.getNbAlert(),
					alert.getReliability(), alert.getCheat()));
		...
		if (ProxyCompanionManager.isIntegrationEnabled()) {
			...
			for (Player pl : ada.getOnlinePlayers()) {
				...
				if (permissionEvent.hasBasicPerm()) {
					if (ada.getConfig().getBoolean("alert.complex", true)) {
						ada.sendMessageRunnableHover(pl,
								Messages.getMessage(pl, alert.getAlertMessageKey(), "%name%", p.getName(), "%cheat%", c.getName(), "%reliability%", reliability, "%nb%", alert.getNbAlert()),
						...
						Messages.sendMessage(pl, alert.getAlertMessageKey(), "%name%", p.getName(), "%cheat%", c.getName(), "%reliability%", reliability, "%nb%", alert.getNbAlert());
					}
					hasPermPeople = true;
				}
			}
	...
	/**
	 * If there is a proxy, and we don't are on proxy platform, the message is sent
	 * to them.
	 * 
	...
	/**
	 * Load all Negativity's class and content. Must to be run after setting adapter
	 */
	...
	private static <T> void safelyLoadExtensions(Class<T> extensionClass, ClassLoader classLoader, Predicate<T> extensionConsumer, Adapter adapter) {
		for (T extension : ServiceLoader.load(extensionClass, classLoader)) {
			try {
				if (extension instanceof PlatformDependentExtension && !((PlatformDependentExtension) extension).getPlatforms().contains(adapter.getPlatformID())) {
					continue;
				}
