	...
	private void downloadAndInstallMysqlDriverIfNeeded() {
		...
		if (Files.exists(mysqlDriverPath)) {
			try {
				...
				getLogger().info("Loaded previously downloaded MySQL driver ...");
				return;
			} catch (Exception e) {
				getLogger().error("Failed add existing MySQL driver to classpath: {}", mysqlDriverPath, e);
			}
		} else {
		...
		try {
			...
			try (InputStream inputStream = connection.getInputStream(); DigestInputStream digestInputStream = new DigestInputStream(inputStream, messageDigest)) {
